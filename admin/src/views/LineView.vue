...
async function fetchStats() {
  try {
    const res = await api.get('/line/stats')
...
async function handleSubmit() {
  ...
  try {
    if (editingId.value) {
      ...
      await api.put(`/line/${editingId.value}`, update)
      ElMessage.success('更新成功')
    } else {
      await api.post('/line', form.value)
...
async function handleDelete(id) {
  await ElMessageBox.confirm('确定要删除该LINE账号吗？', '提示', { type: 'warning' })
  await api.delete(`/line/${id}`)
...
async function toggleStatus(item) {
  if (item.status === 'active') {
    await api.put(`/line/${item._id}`, { status: 'disabled' })
  } else {
    await api.put(`/line/${item._id}/activate`)
  }
  fetchList()
  fetchStats()
}
...
<template>
  <div>
    <div class="page-header">
      <h2 class="page-title">LINE 管理</h2>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑LINE' : '添加LINE'" width="480px">
